...
> **Studio editing direction — OpenReel integration.** The in-product Clip Studio timeline and agent surfaces are inspired by the open-source [OpenReel](https://openreel.video) architecture ([MIT](https://github.com/Augani/openreel-video)): a non-destructive multi-track timeline, a typed editing-tool registry shared by manual and AI edits, a model-agnostic AI agent (OpenAI / Anthropic / local, dry-run planning, undo-a-turn), and hybrid rendering (server `yt-dlp` ingest/transcode + in-browser WebCodecs/WebGPU compositing/export). This design system's pastel timeline palette is reused to mark the AI editing-action stages inside the studio.
