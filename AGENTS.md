...
- **Hero heatmap (`AlignmentGridHero`):** Spring-physics animated shapes (8 Lucide icons) that fly from scattered positions to a 2×4 grid. Uses `packages/shared/src/lib/physics/spring.ts` (damped harmonic oscillator). Dots background via `DotsBackground` component (uniform `opacity=0.7`, 28px grid, `.dots-bg` CSS class). Shapes use `--color-surface-card` background + `--color-hairline` border for dark mode visibility.
- **Global cursor glow (`CursorCircle`):** Fixed-position div rendered in root `layout.tsx`, tracks `document` mousemove via direct DOM manipulation. 120px diameter, `blur(20px)`, `radial-gradient` using `--color-primary`, `pointer-events: none`. Visible on all pages.
- **Dots background system:** `DotsBackground` component (`apps/web/src/presentation/components/layout/DotsBackground.tsx`) wraps `.dots-bg` CSS class. All 17 instances across pages use uniform `opacity={0.7}`. CSS gradient: light mode `hsl(30 10% 80% / 0.7)`, dark mode `hsl(240 10% 25% / 0.8)`, 28px grid, 1px dots.
