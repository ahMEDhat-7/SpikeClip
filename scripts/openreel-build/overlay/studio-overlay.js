...
(function () {
  ...
  var PLATFORM_PRESETS = {
    "youtube-shorts":  { width: 1080, height: 1920, frameRate: 30, maxDuration: 60 },
    "instagram-reels": { width: 1080, height: 1920, frameRate: 30, maxDuration: 90 },
    "tiktok":          { width: 1080, height: 1920, frameRate: 30, maxDuration: 180 }
  };
  ...
  async function loadMediaViaStore(store, url, start, end, platform) {
    var res = await fetch(url);
    if (!res.ok) throw new Error("Failed to fetch source (" + res.status + ")");
    var blob = await res.blob();
    var file = new File([blob], "source.mp4", { type: blob.type || "video/mp4" });

    var preset = PLATFORM_PRESETS[platform] || PLATFORM_PRESETS["youtube-shorts"];
  ...
  function setup(store) {
    ...
    var onMessage = function (event) {
      ...
      if (data.type === "openreel:loadMedia" && data.url) {
        ...
        var loadPlatform = data.platform || "youtube-shorts";
        ...
        var loadFn = hasStoreMethods
          ? loadMediaViaStore(store, data.url, loadStart, loadEnd, loadPlatform)
